...
frec=8
numero_filtros=20
world=users #other o users y others para hacer pruebas
...
compute_lp() {
    ...
    for filename in $(cat $*); do
        ...
        EXEC="wav2lp $lpc_order $db/$filename.wav $w/$FEAT/$filename.$FEAT"          #cuando este acabado, cambiar el 8
...
compute_lpcc() {
    ...
    for filename in $(cat $*); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2lpcc $lpc_order $cepstrum_order $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
compute_mfcc() {
    ...
    for filename in $(cat $*); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2mfcc $mfcc_order $frec $numero_filtros $db/$filename.wav $w/$FEAT/$filename.$FEAT"            #falta cambiar el 8 por el order que toque
        echo $EXEC && $EXEC || exit 1
    done
}
